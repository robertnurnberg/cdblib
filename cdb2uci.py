...
class Engine:
    def __init__(self, args):
        self.cdb = cdblib.cdbAPI(user=VERSION, showErrors=False)
    ...
    def go(self):
        ...
        r = self.get_movelist(tb_with_cr)
        if tb_with_cr:
            depth = nodes = 0
            r[0][1] = "cp 0"
            print(
        ...
        if self.querypv:
            for idx in range(multipv):
                self.board.push_uci(r[idx][0])
                q = self.cdb.querypvstable(self.board.epd())
                self.board.pop()
                pv = cdblib.json2pv(q)
                if pv:
                    nodes += pv.count(" ") + 1
