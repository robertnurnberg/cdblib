...
parser = argparse.ArgumentParser(description="A simple script to pass pgns to chessdb.cn.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
gamelist = []
while game := chess.pgn.read_game(pgn): gamelist.append(game)
...
for i in reversed(range(gn)):
    ...
    for move in gamelist[i].mainline_moves():
        ...
        if plies >= args.depth or pc <= 7: break # cdb only stores pos if >= 8
    if args.verbose: 
        print(f"  For pgn {i+1}/{gn} read {plies}/{args.depth} plies. Final pos has {pc} pieces.", flush=True)
    if plies == 0: continue # some pgn's include empty games
    ...
    if r["status"] == "ok": 
        seen += 1
        if args.verbose: 
            print(f"    Position at depth {plies} is already in chessdb.cn"
                  + (f", with distance {cdbply} from root." if cdbply != -1 else
...
print(f"Queued {db.queued} new positions to chessdb.cn. Local cache hit rate: {db.req_cached}/{db.req_received} = {db.req_cached/db.req_received*100:.2f}%.")
