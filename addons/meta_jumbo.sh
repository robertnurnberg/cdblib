...
total_lines=1
while [ $total_lines -ne 0 ]; do
    if [[ ! -f $meta_unknown ]]; then
        grep -v "cdb eval:" "$meta_all" >"$meta_unknown" || true
        total_lines=$(wc -l <"$meta_unknown")
        echo "Stored $total_lines unknown positions in '$meta_unknown'."
        if [ $total_lines -ne 0 ]; then
            echo "Parsing these with jumbo_fens2cdb.sh --quick to cdb ..."
        fi
    else
        total_lines=$(wc -l <"$meta_unknown")
        echo "Found '$meta_unknown' with $total_lines unknown positions."
        if [ $total_lines -ne 0 ]; then
    ...
    if [ $total_lines -ne 0 ]; then
        ...
        tmpname="$(mktemp -q)"
        echo "Incorporating the new scores from '$meta_unknown_cdb' into '$meta_all' ..."
